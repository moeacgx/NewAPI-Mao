...
func ValidateRequestURL(requestURL, baseURL string, allowedHosts []string) error {
	...
	if err != nil || (request.Scheme != "https" && request.Scheme != "http") || request.Host == "" || request.User != nil {
		return fmt.Errorf("plugin request URL must be absolute HTTP(S); userinfo is not allowed")
	}
	...
	if requestHost == canonicalHost(base) && request.Scheme == base.Scheme {
		return nil
	}
