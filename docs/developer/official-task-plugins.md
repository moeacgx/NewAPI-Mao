...
任务插件渠道不支持通用聊天端点探测，返回本地不支持测试结果，不发送上游请求、不计费。
Jev 使用 `POST /v1/systemone`，可在专用测试工具中验证；批量通用测试不支持结果不是
供应商成功证明。见[工作记录](../workflows/2026-09/24_task_plugin_channel_test_guard.md)。
