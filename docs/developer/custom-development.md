...
- 主程序仅按上游响应正文声明的模型名校验，保留规则分组、请求模型、渠道白名单、连续容错及异常关渠。
- faster-model 响应头识别已撤销，不再采集头部模型或写入专用日志及通知来源字段；现有外置模块 0.2.1 无需升级，历史数据与已关闭渠道保留。
- 记录：[撤销说明](../workflows/2026-09/21_remove_codex_faster_model_evidence.md)。正文声明不能独立证明底层模型身份。
