...
func (a *TaskAdaptor) DoRequest(c *gin.Context, info *relaycommon.RelayInfo, body io.Reader) (*http.Response, error) {
	...
	method := strings.ToUpper(strings.TrimSpace(a.submit.Method))
	if method == "" {
		method = http.MethodPost
	}
	req, err := http.NewRequestWithContext(c.Request.Context(), method, requestURL, body)
	if err != nil {
		return nil, err
	}
...
func (a *TaskAdaptor) ParseResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (_ *channel.TaskSubmitResponse, taskErr *dto.TaskError) {
	...
	if err != nil {
		logger.LogWarn(c, fmt.Sprintf("task plugin %s completion context failed; retaining reserved quota", a.plugin.Meta.Key))
		return response, nil
	}
	facts, err := a.plugin.Engine.Call(c.Request.Context(), "extractUsageOnComplete", ctx, jsonValue(immediate), parsed.TaskData)
	if err != nil {
		logger.LogWarn(c, fmt.Sprintf("task plugin %s completion usage failed; retaining reserved quota", a.plugin.Meta.Key))
		return response, nil
	}
...
var _ channel.TaskAdaptor = (*Legacy)(nil)
