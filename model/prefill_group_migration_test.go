...
func TestPrefillMigrationQuotesLiteralObjectNames(t *testing.T) {
	...
	require.NoError(t, dropConflictingPrefillGroupUniqueness(db, conflictingPrefillGroupUniqueness{
		schemaName: `schema.name`, tableName: `prefill_groups`,
		constraints: []string{`legacy.name`, `legacy "name"`},
		indexes:     []string{`other_schema.keep_index`, `literal";DROP TABLE users;--`},
	}))
	assert.Equal(t, []string{
		`ALTER TABLE "schema.name"."prefill_groups" DROP CONSTRAINT "legacy.name"`,
		`ALTER TABLE "schema.name"."prefill_groups" DROP CONSTRAINT "legacy ""name"""`,
		`DROP INDEX "schema.name"."other_schema.keep_index"`,
		`DROP INDEX "schema.name"."literal"";DROP TABLE users;--"`,
	}, recorder.statements)
}
...
func testPrefillGroupMigrationNonPostgreSQL(t *testing.T, db *gorm.DB) {
	...
	for range 2 {
		...
		assert.Empty(t, recorder.schemaMutations(), "非 PostgreSQL 迁移不能修改表结构")
		for _, statement := range recorder.statements {
			assert.NotRegexp(t, `(?i)^\s*(CREATE|DROP|ALTER)\s`, statement, "非 PostgreSQL 迁移不能修改约束或索引")
		}
	}
...
func TestMigratePrefillGroupUniquenessPostgreSQL(t *testing.T) {
	...
	// 使用当前 chooseDB 的预处理策略；pooler/JSON 参数编码联动修复不在本阶段。
	...
	tests := []struct {
		...
		verifyPreserved     func(*testing.T, *gorm.DB)
		wantError           string
	}{
		{name: "fresh"},
		{
		...
		{
			name:        "literal_dotted_names_preserve_other_schema_index",
			constraints: []string{"legacy.name"},
			indexes:     []string{"pg_temp.keep_index"},
			prepareOld: func(t *testing.T, tx *gorm.DB) {
				t.Helper()
				require.NoError(t, tx.Exec("CREATE TEMP TABLE unrelated_prefill_reference (name text)").Error)
				require.NoError(t, tx.Exec("CREATE INDEX keep_index ON unrelated_prefill_reference (name)").Error)
			},
			verifyPreserved: func(t *testing.T, tx *gorm.DB) {
				t.Helper()
				var exists bool
				require.NoError(t, tx.Raw("SELECT to_regclass('pg_temp.keep_index') IS NOT NULL").Scan(&exists).Error)
				assert.True(t, exists, "不得把字面点号索引名解析成其他 schema 的索引")
			},
		},
		{
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, constraintName := range test.constraints {
				require.NoError(t, tx.Exec(
					...
					clause.Table{Name: "prefill_groups"},
					clause.Column{Name: `"` + strings.ReplaceAll(constraintName, `"`, `""`) + `"`, Raw: true},
					clause.Column{Name: "name"},
				).Error)
			}
			for _, indexName := range test.indexes {
				require.NoError(t, tx.Exec(
					"CREATE UNIQUE INDEX ? ON ? (?)",
					clause.Column{Name: `"` + strings.ReplaceAll(indexName, `"`, `""`) + `"`, Raw: true},
					clause.Table{Name: "prefill_groups"},
					clause.Column{Name: "name"},
				).Error)
			}
...
ORDER BY indexname`).Scan(&restoredIndexes).Error)
			...
			for pass := range 2 {
				...
				if pass == 1 {
					...
					for _, statement := range recorder.statements {
						assert.NotRegexp(t, `(?i)^\s*(CREATE|DROP|ALTER)\s`, statement, "重复启动不能重建索引或约束")
					}
				}
			}
...
WHERE schemaname = current_schema() AND indexname = ?`, oldIndex.Name).Scan(&definition).Error)
			...
			assert.Equal(t, original, preserved)
			if test.verifyPreserved != nil {
				test.verifyPreserved(t, tx)
			}
