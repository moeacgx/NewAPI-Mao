...
func TestPluginNativeRouteUsesBearerCORS(t *testing.T) {
	...
	plugin := compileRouterPlugin(t, "cors-owner", "1.0.0", `[{method:"POST", path:"/vendor/decisions", type:"submit"}]`)
	require.NoError(t, jsplugin.DefaultRegistry.ReplaceOverrides([]*jsplugin.LoadedPlugin{plugin}))
	outer := gin.New()
	outer.Use(corsPreflightBoundary())
	outer.NoRoute(pathAwareCORS())
	request := httptest.NewRequest(http.MethodOptions, "/vendor/decisions", nil)
	...
	assert.False(t, isRelayCORSPath("/api/private"))
}
