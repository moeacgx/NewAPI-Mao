...
test.each([
  ...
  '选择名称=$name 和额度=$quota 时按选项导出 TXT 与 Markdown',
  async (options) => {
...
test('名称包含表格分隔符和标记时仍保持在单个 Markdown 单元格', async () => {
  const downloads = captureDownloads()
  const user = userEvent.setup()
  render(
    <RedemptionsExportDialog
...
test('批量创建成功后使用返回兑换码及配置货币展示导出', async () => {
  const downloads = captureDownloads()
  const user = userEvent.setup()
  ...
  const create = vi.spyOn(api, 'post').mockResolvedValue({
  ...
  fireEvent.change(within(createDialog).getByLabelText('Redeem Limit'), {
    target: { value: '3' },
  })
  await user.click(
  ...
  expect(create).toHaveBeenCalledWith(
    '/api/redemption/',
    expect.objectContaining({ count: 2, max_redeem_count: 3 })
  )
  await waitFor(() =>
    expect(
...
test('默认不勾选文件导出时完成后关闭且不下载', async () => {
  const downloads = captureDownloads()
  const user = userEvent.setup()
  const onClose = vi.fn()
  render(
