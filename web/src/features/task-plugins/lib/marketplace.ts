...
export function resolvePluginSourceUrl(
  ...
  if (
    ...
    !resolved.pathname.startsWith(directory)
  ) {
    return null
  }
...
export function parseMarketplaceIndex(payload: unknown): MarketplaceIndex {
  ...
  const indexVersion = raw.indexVersion
  ...
  if (!Array.isArray(raw.plugins) || raw.plugins.length > 500) {
    throw new Error('invalid marketplace plugin list')
  }
...
function parseMarketplacePlugin(entry: unknown): MarketplacePlugin | null {
  ...
  if (Array.isArray(raw.versions)) {
    for (const candidate of raw.versions.slice(0, 64)) {
...
export const MAOLAO_MARKETPLACE_INDEX_URL =
  'https://raw.githubusercontent.com/moeacgx/maolaonewapi-plugins/main/index.json'
