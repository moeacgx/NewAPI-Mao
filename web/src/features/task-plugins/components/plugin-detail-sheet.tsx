...
import { PluginVersionSelect as Combobox } from './plugin-version-select'
...
function PluginDetailContent(props: {
  ...
  const compareQuery = useQuery({
    ...
    enabled: props.canManage && Boolean(key && compareVersion),
  })
      ...
      >
        <div className='shrink-0 overflow-x-auto border-b px-4 sm:px-6'>
          ...
          >
            ...
            <TabsTrigger value='source' disabled={!props.canManage}>
              {t('Plugin source')}
            </TabsTrigger>
            <TabsTrigger value='versions'>{t('Version history')}</TabsTrigger>
            <TabsTrigger value='diff' disabled={!props.canManage}>
              {t('Source diff')}
            </TabsTrigger>
            <TabsTrigger value='sandbox' disabled>
              {t('Plugin sandbox')}
            </TabsTrigger>
          </TabsList>
        </div>
