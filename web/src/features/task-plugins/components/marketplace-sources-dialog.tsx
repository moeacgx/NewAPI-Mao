...
import { validMarketplaceIndexUrl } from '../lib/marketplace-download'
...
export function MarketplaceSourcesDialog(props: MarketplaceSourcesDialogProps) {
  ...
  useEffect(() => {
    if (!props.open) seeded.current = false
    if (props.open && sourcesQuery.data && !seeded.current) {
      seeded.current = true
      setDraft(
  ...
  const invalidRow =
    draft.length > 16 ||
    new Set(draft.map((row) => row.index_url.trim())).size !== draft.length ||
    draft.some(
      (row) =>
        !row.name.trim() ||
        [...row.name.trim()].length > 128 ||
        !validMarketplaceIndexUrl(row.index_url.trim())
    )
      ...
      <DialogContent className='max-h-[90vh] overflow-y-auto sm:max-w-2xl'>
        ...
        <div className='space-y-3'>
          {sourcesQuery.isError && (
            <p role='alert'>{t('Could not load marketplace sources')}</p>
          )}
          {draft.length === 0 && sourcesQuery.isSuccess && (
            ...
            <div key={row.rowId} className='space-y-2 rounded-md border p-3'>
              <div className='flex items-center justify-between gap-2'>
                ...
                <div className='flex items-center gap-2'>
                  {row.index_url === MAOLAO_MARKETPLACE_INDEX_URL && (
        ...
        <DialogFooter>
          ...
          <Button
            disabled={
              invalidRow || !sourcesQuery.isSuccess || saveMutation.isPending
            }
            onClick={() =>
              saveMutation.mutate(
                draft.map((row) => ({
