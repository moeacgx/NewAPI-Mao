...
test('invalid source documents are errors rather than an empty marketplace', () => {
  assert.throws(() => parseMarketplaceIndex({ indexVersion: 1 }))
  assert.throws(() => parseMarketplaceIndex({ indexVersion: '1', plugins: [] }))
  assert.throws(() => parseMarketplaceIndex({ indexVersion: 1, plugins: {} }))
  assert.deepEqual(
    parseMarketplaceIndex({ indexVersion: 1, plugins: [] }).plugins,
    []
  )
})

test('encoded whitespace and empty source path segments cannot bypass the install contract', () => {
  for (const path of [
    'tasks//plugin.js',
    'tasks/%0aplugin.js',
    'tasks/%20plugin.js',
    ...
    assert.equal(resolvePluginSourceUrl(OFFICIAL_INDEX_URL, path), null)
  }
})
...
describe('marketplace source path resolution', () => {
  ...
  test('rejects a root-relative path outside the index directory', () => {
    assert.equal(
      resolvePluginSourceUrl(
        'https://host.example/x/index.json',
        '/other/plugin.js'
