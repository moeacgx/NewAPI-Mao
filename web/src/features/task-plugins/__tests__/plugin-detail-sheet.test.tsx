...
test('sandbox remains unavailable without sending a dry-run request', async () => {
  renderSheet({})
  expect(screen.getByRole('tab', { name: 'Plugin sandbox' })).toHaveAttribute(
    'aria-disabled',
    'true'
  )
})
...
test('comparison selection shows loading, retries failure, and resets after reopening', async () => {
  ...
  await user.selectOptions(selector, '1.0.0')
  expect(screen.getByText('Loading...')).toBeVisible()
