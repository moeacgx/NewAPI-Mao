...
func TaskExecutionSnapshotFromContext(ctx *gin.Context) *model.TaskExecutionSnapshot {
	...
	if pin, ok := ctx.Get("task_plugin_snapshot"); ok {
		if typed, valid := pin.(*model.TaskPluginSnapshot); valid {
			snapshot.TaskPlugin = typed
		}
	}
...
func AppendTaskPluginAuditInfo(other map[string]interface{}, snapshot *model.TaskPluginSnapshot) {
	...
	admin["task_plugin"] = taskPlugin
	root, _ := other["root_info"].(map[string]interface{})
	if root == nil {
		root = make(map[string]interface{})
		other["root_info"] = root
	}
	root["task_plugin"] = map[string]any{
		"key":         snapshot.Key,
		"version":     snapshot.Version,
